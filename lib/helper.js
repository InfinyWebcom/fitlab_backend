...
const sendmail = (data) => {
    try {
        let smtpTransport = nodemailer.createTransport({
            ...
            requiresAuth: true,
            auth: {
                // user:'testinfiny873@gmail.com',
                // pass:'vmddsraaqxxpxcqf'
...
const sendEmailstoClient = (user) => {
    const { email,name,subject,your_message} = user;
    // const otp = Math.floor(100000 + Math.random() * 900000);
    const data = {
        title: `Contact for Information About Fitlab`,
        name,
        userEmail:email,
        email:"developer@infiny.in",
        subject,
        your_message,
        year: moment().get('year')
    }
    console.log("jjffffffffjfjfjjfjfjfjffjfjfjfjfj",data)
    const emailContent = fs.readFileSync(process.cwd() +'/views/userDetails.ejs', 'utf8');
    console.log("jjffffffffjfjfjjfjfjfjffjfjfjfjfj",emailContent)
    const emailHtml = ejs.render(emailContent, data);
    var mailData = {
        email:"developer@infiny.in",
        subject,
    ...
    console.log("dddddddddddddddddddddddddddddd",mailData)
    sendmail(mailData);
    return mailData;
}
const sendInquieryEmailstoClient=(user)=>{
    const {  email, } = user;
    // const otp = Math.floor(100000 + Math.random() * 900000);
    const data = {
        title: `Contact for Information About Fitlab`,
    ...
    const emailContent = fs.readFileSync(process.cwd() +'/views/informationuser.ejs', 'utf8');
    const emailHtml = ejs.render(emailContent, data);
    var mailData = {
        email:"developer@infiny.in",
        subject:'Email for Enquiry ',
        body: emailHtml,
    };
    console.log("dddddddddddddddddddddddddddddd",mailData)
    // sendmail(mailData);
